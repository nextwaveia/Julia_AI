...
export class CompressionEngine {
   ...
   * Compression: 75-85%
   */
  compressStatsExtraction(
    input: string,
    stats: { agents: number; files: number; compression: number }
  ): CompressionResult {
  ...
  selectBestStrategy(
    input: string,
    goal: 'quality' | 'speed' | 'balance'
  ): string {
    if (goal === 'quality') return 'stats-extraction';
    if (goal === 'speed') return 'error-only';
    return 'hybrid';
  }
}
