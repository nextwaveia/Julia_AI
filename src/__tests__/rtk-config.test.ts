import { RTKConfigManager } from '../rtk/rtk-config';

describe('RTKConfigManager', () => {
  let manager: RTKConfigManager;

  beforeEach(() => {
    manager = new RTKConfigManager();
  });
  ...
  test('should deserialize from JSON', () => {
    const json = manager.toJSON();
    const newManager = RTKConfigManager.fromJSON(json);
  ...
  test('should throw on invalid JSON', () => {
    expect(() => {
      RTKConfigManager.fromJSON('invalid json');
    }).toThrow();
  });

  test('should handle configuration overrides', () => {
    const customManager = new RTKConfigManager({
