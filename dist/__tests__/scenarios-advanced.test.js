...
describe('Advanced Scenario Tests', () => {
    ...
    test('Scenario: Edge case - very short input', async () => {
        const result = await (0, index_1.orchestrate)('Fix');
    ...
    test('Scenario: Refactor with multiple modules', async () => {
        const result = await (0, index_1.orchestrate)('Refactor auth module and API endpoints');
    ...
    test('Scenario: Audit with validation', async () => {
        const result = await (0, index_1.orchestrate)('Audit RBAC settings compliance');
    ...
    test('Scenario: Performance sensitive task', async () => {
        ...
        const result = await (0, index_1.orchestrate)('Enhance performance');
    ...
    test('Scenario: All agents executed', async () => {
        const result = await (0, index_1.orchestrate)('Implementar settings.team');
        const executedCount = result.results.filter((r) => r.status === 'success').length;
...
describe('Stress Test Scenarios', () => {
    test('Stress: Multiple sequential scenarios', async () => {
        ...
        for (const scenario of scenarios) {
            const result = await (0, index_1.orchestrate)(scenario);
            expect(result.report.status).toBe('success');
        }
    });
    ...
    test('Stress: Large input handling', async () => {
        ...
        const result = await (0, index_1.orchestrate)(`Implementar ${longTarget}`);
...
describe('Error Scenario Handling', () => {
    test('Error: Graceful handling of empty action', async () => {
        const result = await (0, index_1.orchestrate)('settings.team');
        expect(result.intent.action).toBe('implement');
    });
    test('Error: Unknown action defaults to implement', async () => {
        const result = await (0, index_1.orchestrate)('Unknown action for settings.team');
        expect(result.intent.action).toBe('implement');
    });
    ...
    test('Error: Numeric input handling', async () => {
        const result = await (0, index_1.orchestrate)('Implementar 123.456');
        expect(result.intent.target).toBeTruthy();
    });
    test('Error: UTF-8 character handling', async () => {
        const result = await (0, index_1.orchestrate)('Implementar settings.équipe');
        expect(result.intent).toBeTruthy();
    });
});
//# sourceMappingURL=scenarios-advanced.test.js.map