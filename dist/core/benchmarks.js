...
async function runBenchmarks() {
    ...
    for (const scenario of scenarios) {
        ...
        const successRate = result.results.filter((r) => r.status === 'success').length /
...
╠════════════════════════╬═══════════╬═══════════╬══════════╬═════════╬════════════╣`;
    const rows = results
        .map((r) => `║ ${r.scenario.padEnd(22)} ║ ${String(r.executionTime).padStart(9)} ║ ${String(r.tokenCompression).padStart(9)} ║ ${r.parallelismFactor.toFixed(2).padStart(8)} ║ ${String(r.taskCount).padStart(7)} ║ ${(r.successRate * 100).toFixed(1).padStart(10)} ║`)
        .join('\n');
    const avg = {
        time: results.reduce((sum, r) => sum + r.executionTime, 0) / results.length,
        compression: results.reduce((sum, r) => sum + r.tokenCompression, 0) / results.length,
        parallelism: results.reduce((sum, r) => sum + r.parallelismFactor, 0) / results.length,
    };
    const footer = `╠════════════════════════╬═══════════╬═══════════╬══════════╬═════════╬════════════╣
